...
ml_beta1 <- solve(crossprod(B1),crossprod(B1,y))
ml_beta1

ml_beta2 <- solve(crossprod(B2),crossprod(B2,y))
ml_beta2

sigma_b <- numeric()
sigma_b_sd <- numeric()

# Getting the maximum range of coefficients in the splines.
for(ii in 1:10){
  form_ <- formula(paste0("y ~  bs(x.",ii,")"))
  mod <- mgcv::gam(form_, data = dat_df)
  summary(mod)
  sigma_b[ii] <- max(abs(mod$coefficients[2:4]))
  summ_ <- summary(mod)
  sigma_b_sd[ii] <- max(summ_$se[2:4])
  sigma_b
}


ll1 <- dmvn(y, rep(0, n), diag(sigma^2, n) + sigma_b[b1_var]^2 * B1 %*% t(B1), log = TRUE)
ll2 <- dmvn(y, rep(0, n), diag(sigma^2, n) + sigma_b[b2_var]^2 * B2 %*% t(B2), log = TRUE)
...
# Doing experiments considering interactions
...
ll1 <- dmvn(y, rep(0, n), diag(sigma^2, n) + sigma_b[b1_var]^2 * B1 %*% t(B1), log = TRUE) # Likelihood of only having the main effect
ll2 <- dmvn(y, rep(0, n), diag(sigma^2, n) + sigma_b[b1_inter_]^2 * B1_B2 %*% t(B1_B2), log = TRUE) # Likelihood of having the interaction only
ll3 <- dmvn(y, rep(0, n), diag(sigma^2, n) + sigma_b[b1_var]^2 * B1 %*% t(B1) + sigma_b[b1_inter_]^2 * B1_B2 %*% t(B1_B2), log = TRUE) # Likelihood plus main effect + one interaction
