...
for(jj in 1:(NCOL(x_train)+1)){
  ...
  if(jj <= NCOL(x_train)){
    ...
    scatterplot3d::scatterplot3d(x_train[,1], x_train[,2],
      ...
      scatterplot3d::scatterplot3d(x_test[,2], x_test[,3],
                                    (colMeans(main_effects_test_list_norm[[6]][n_burn_plot:n_mcmc,,drop = FALSE])),
                                   xlab = "X.1", ylab = "X.2", zlab = "f(x.1) + f (x.2) + f(x.1,x.2)", pch = 19)
...
# text(x =1:6,
#      names = c(colnames(rsp_mod$data$x_train), "solar:wind", "solar:temp","wind:temp"))
